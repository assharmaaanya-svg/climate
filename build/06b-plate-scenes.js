...
const STAR_CARD = {
  ...
  alkaid: {
    ...
    /* NAME THE SONG, AND SAY PLAINLY WHAT IT WAS FOR.
       The old line ended on "passed on in song to guide people escaping slavery", which
       buried the most important thing on this card in a subordinate clause and never said
       which song. It is the reason Alkaid is in the piece at all. */
    body: "The shape it ends was called the Drinking Gourd, and people escaping slavery in America followed it north to freedom. The song Follow the Drinkin’ Gourd carried those directions, keeping the way out in something that could be sung instead of written down.",
...
/* THE ORDER IS THE ARGUMENT.
   The illustration opens it, the way a small drawing opens a page; then the name; then
   one quiet line saying what it is in plain words; then the writing. Nothing between
   them but space. What is gone from here is everything that made it a database record:
   the catalogue designation, the coordinate-style location line, and the definition-list
   grid of magnitudes at the foot. */
stEl.innerHTML =
...
document.body.appendChild(stEl);
{
  ...
  /* IT BELONGS TO THE MEMORY, NOT TO AN ATLAS.
     What was here was a blue-violet panel with blue-white type, a blue hairline rim, a
     lit lens edge along the top, two hard rules across it, four competing type styles
     stacked in the header, and a definition-list of magnitudes at the foot. Every one
     of those belongs to astronomy software. The writing was already the good part and
     the card was arguing with it.

     So the ground is smoky warm near-black — the same darkness the rest of the piece
     is lit against, not a slice of nebula — the type is the interface ivory, and the
  ...
  #starcard{
    ...
    color:#eee7d9;
    /* one warm ember low on the left, and smoke over it. No second hue. */
    background:
      radial-gradient(115% 95% at 6% 4%, rgba(74,54,36,.42), transparent 64%),
      linear-gradient(168deg, rgba(21,18,16,.80), rgba(11,10,9,.87));
    border:1px solid rgba(240,226,198,.09);
    border-radius:13px;
    -webkit-backdrop-filter:blur(22px) saturate(1.04); backdrop-filter:blur(22px) saturate(1.04);
    /* low and soft. It was a 90px spread at 60% black, which reads as a floating
       dashboard widget rather than as something resting on the night. */
    box-shadow:0 18px 52px rgba(0,0,0,.46);
    scrollbar-width:thin; scrollbar-color:rgba(240,226,198,.16) transparent;
  }
  #starcard::-webkit-scrollbar{ width:3px; }
  #starcard::-webkit-scrollbar-thumb{ background:rgba(240,226,198,.16); border-radius:3px; }
  #starcard.dockL{ left:1.9rem; right:auto; }
  #starcard.dockR{ right:1.9rem; left:auto; }
  #starcard.on{ opacity:1; transform:translateY(0); pointer-events:auto;
  ...
  /* the close button: discoverable, and nowhere near the strongest thing on the card.
     The filled circle and its ring are gone; it is a stroke that warms on hover. */
  #starcard .x{ position:absolute; top:.95rem; right:1rem; width:30px; height:30px;
    display:grid; place-items:center; padding:0; cursor:pointer;
    background:none; border:0; border-radius:50%;
    color:rgba(238,231,217,.42); transition:color .22s, background .22s; }
  #starcard .x svg{ width:13px; height:13px; stroke:currentColor; stroke-width:1.35;
  ...
  #starcard .body p{ margin:0 0 .8rem; font-size:clamp(.87rem,2.2vw,.935rem); line-height:1.66;
  ...
  /* THE CONSTELLATION.
     ...
     with round ends, and they bow very slightly instead of running dead straight — a
     shape somebody remembers rather than one plotted off a chart. The stars keep a
     faint halo so they read as light rather than as data points, and the star the
     visitor touched keeps its gold, which is the whole reason the drawing is here: it
     ties the star in the sky to its place in the shape. */
  #starcard .chart .lnk{ stroke:rgba(238,228,206,.26); stroke-width:.75; fill:none;
    stroke-linecap:round; }
  #starcard .chart .glow{ fill:rgba(246,240,226,.10); }
  #starcard .chart .st{ fill:#f2ebda; }
  #starcard .chart .me{ fill:#ffeec9; }
  #starcard .chart .meglow{ fill:rgba(217,182,120,.20); }
  #starcard .chart .halo{ fill:none; stroke:rgba(217,182,120,.72); stroke-width:.95; }
...
function starChart(id){
  ...
  /* the frame grew, so the mapping grew with it */
  const px = v => 13 + (v-x0)/(x1-x0||1) * 106;
  const py = v => 13 + (v-y0)/(y1-y0||1) * 54;
  let out = "";

  /* THE LINES BOW, VERY SLIGHTLY, AND ALWAYS THE SAME WAY.
     A ruled straight line between two stars is a chart; a line with a little give in it
     is a shape somebody is recalling. The bow is a quadratic whose control point is
     pushed off the midpoint perpendicular to the run, by an amount taken from a hash of
     the two endpoints — so it is deterministic and identical on every visit, never
     wobbling or animating, and it is around one unit on a frame a hundred and thirty
     wide. Enough to feel drawn. Not enough to look inaccurate: the stars themselves are
     still exactly where the sky puts them, and only the line between them relaxes. */
  for (const [a2,b2] of DIP_LINKS){
    const ax = px(DIPPER[a2].x), ay = py(DIPPER[a2].y);
    const bx = px(DIPPER[b2].x), by = py(DIPPER[b2].y);
    const mx = (ax+bx)/2, my = (ay+by)/2;
    const dx = bx-ax, dy = by-ay, len = Math.hypot(dx,dy) || 1;
    const bend = (hash(a2*7.13 + b2*3.71) - 0.5) * 2.4;
    const cx = mx - (dy/len)*bend, cy = my + (dx/len)*bend;
    out += `<path class="lnk" d="M${ax.toFixed(1)} ${ay.toFixed(1)}`
  ...
  for (const s of all){
    ...
    const r = me ? 2.35 : 1.05 + s.b*0.95;
    /* a soft halo under every star so they read as light rather than as plotted points */
    out += `<circle class="${me?"meglow":"glow"}" cx="${X.toFixed(1)}" cy="${Y.toFixed(1)}"`
         + ` r="${(r*2.9).toFixed(2)}"${me?"":` opacity="${(0.34+s.b*0.5).toFixed(2)}"`}/>`;
    out += `<circle class="${me?"me":"st"}" cx="${X.toFixed(1)}" cy="${Y.toFixed(1)}"`
         + ` r="${r.toFixed(2)}"${me?"":` opacity="${(0.34+s.b*0.44).toFixed(2)}"`}/>`;
    if (me) out += `<circle class="halo" cx="${X.toFixed(1)}" cy="${Y.toFixed(1)}" r="7"/>`;
    /* Alcor rides beside Mizar, which is the whole reason that card exists */
    if (s.id === "mizar")
      out += `<circle class="${id==="mizar"?"me":"st"}" cx="${(X+3.9).toFixed(1)}"`
           + ` cy="${(Y-3.0).toFixed(1)}" r="${id==="mizar"?1.4:0.95}"`
           + `${id==="mizar"?"":' opacity="0.48"'}/>`;
  }
  return out;
}
...
function fillStarCard(id){
  ...
  stEl.querySelector("h2.nm").textContent = c.name;
  /* the descriptor is its own line now instead of a caps label bolted under the name.
     Not every star has one, and the rule for it collapses when it is empty. */
  stEl.querySelector(".sub").textContent = c.sub || "";
  stEl.querySelector(".lede").textContent = c.lede;
  stEl.querySelector(".body").innerHTML =
    c.body.split("\n\n").map(p => "<p>" + esc(p) + "</p>").join("");
  /* WHAT THE CARD NO LONGER SHOWS: `where`, `bayer`, and `facts`.
     The location line was coordinate-styled small caps, the Bayer designation is a
     catalogue key, and `facts` was a definition list of magnitudes and distances under
     uppercase labels. Between them they were most of what made this read as a database
     record. The drawing above says where the star is, and it says it better.

     `facts` went entirely rather than being kept faint at the foot. Demoting it to a
     quiet line was the first attempt and it was worse than either option: still the one
     surviving piece of catalogue metadata, and now sitting under the scroll fade where
     it could not quite be read. The card is stronger ending on the memory. Every one of
     these fields is still in the data untouched — nothing has been rewritten, and they
     are one line away from coming back. */
  stEl.scrollTop = 0;
  /* THE FADE ONLY EXISTS WHEN THERE IS SOMETHING UNDER IT.
     It is a sticky gradient with a negative margin, so it always covered the last few
     lines whether or not the card could scroll — which on a short card meant a permanent
     smear over the end of the writing. Measured after filling, and off unless the
...
/* The lookout chapter that used to live here — the binoculars — is now its own
   file, build/06g-lookout.js. It grew past the point where it belonged in a
   collection of plate scenes. */
