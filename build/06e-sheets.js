...
/* ------------------------------------------------------------- the mother
   She is a person standing behind a sheet, not a piece of washing. Everything
   she is made of used to go through the same cloth mesh as the sheets, which is
   why she looked tangled in them: her skirt rippled on the sheet's wave, at the
   sheet's frequency, and moved when the visitor dragged. A skirt does move, but
   it moves because she is standing in a field in the evening — not because
   somebody pulled the line she is behind.

   So nothing of her is warped now. Her shadow is drawn straight, and her skirt
   swings from her waist as one piece, on its own slow clock, with a shear
   rather than a ripple. Two sprites, two plain draws, and she keeps still.
*/
function drawMother(t, dt, rect, c, air, sheetBox){
  ...
  // her own breeze: slow, small, and nothing to do with the drag
  const swing = Math.sin(t*0.62) * 0.020 + Math.sin(t*0.41 + 1.3) * 0.012;

  /* the shadow on the cloth. Multiply is both the right blend for a shadow and
     the reason her sprite needs no keying — she is a silhouette painted on
     solid white, and white under multiply leaves the backdrop as it was. */
  ctx.save();
  ...
  if (imgReady(torso)){
    ctx.drawImage(torso,
      ...
      rect.x + tb[0]*rect.w, rect.y + tb[1]*rect.h,
      tb[2]*rect.w, tb[3]*rect.h);
  }
  ctx.restore();

  /* her skirt, below the hem. Its size is taken off momwithonesheetsprite.png —
     that is this same sheet with her really behind it, and there the skirt is
     most of the sheet's width and hangs about four tenths of the sheet's height
     below the hem. It swings from the waist as one piece: a shear, easing to
     nothing at the top, so the waist stays put and only the hem travels. */
  if (imgReady(skirt)){
    const sk = SHEETS.skirt.box;
    const skW = sheetBox[2]*0.90, skH = sheetBox[3]*0.60;
    const topY = hemY - skH*0.30;                  // the rest is behind the sheet
    ctx.save();
    ctx.beginPath();
    ctx.rect(rect.x + (midX-skW)*rect.w, rect.y + hemY*rect.h,
             skW*3*rect.w, (1.2-hemY)*rect.h);
    ctx.clip();
    ctx.globalAlpha = a*0.94;
    // shear about the waist: x is displaced in proportion to depth down the skirt
    ctx.translate(rect.x + midX*rect.w, rect.y + topY*rect.h);
    ctx.transform(1, 0, swing*1.6, 1, 0, 0);
    ctx.drawImage(skirt,
      sk[0]*skirt.naturalWidth, sk[1]*skirt.naturalHeight,
      sk[2]*skirt.naturalWidth, sk[3]*skirt.naturalHeight,
      -skW*0.5*rect.w, 0, skW*rect.w, skH*rect.h);
    // the evening is behind her, so the skirt is in its own shade
    ctx.globalCompositeOperation = "source-atop";
    ctx.globalAlpha = a*0.26;
    ctx.fillStyle = rgba([64,34,10], 1);
    ctx.fillRect(-skW*0.5*rect.w, 0, skW*rect.w, skH*rect.h);
    ctx.restore();
  }
}
