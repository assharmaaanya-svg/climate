...
function litSprite(img, key, w, h, dark, warm){
  ...
  if (warm > 0){
    ...
    // multiply against a transparent pixel returns the source, so the fill just
    // painted the sprite's empty margin solid. Redrawing the sprite through
...
/* ---------------------------------------------------------------------------
   ...
   THERE IS NO TORN OPENING IN THIS WALL and there never was — the brief for this asked
   for the drawing to be masked to an existing irregular hole in the plaster, and no such
   hole exists anywhere in the piece. What the complaint was actually describing is real
   though, and it is this function: a sheet of paper drawn as `drawImage(PAPER, x, y, w, h)`,
   which is a rectangle with four hard corners sitting on a painted wall, and that is
   exactly what "a rectangular digital image placed over the wall" looks like.

   So the boundary the brief wanted to mask against is BUILT rather than borrowed: the
   sheet gets its own torn edge. A closed path is walked around the rectangle with the
   deviation coming from `hash`, so it is the same edge every frame and the same edge in
   both bedrooms; it is deliberately quiet on the two long sides, where a sheet is cut, and
   loosest at the corners and the bottom, where one comes away from a wall. The artwork is
   drawn oversize behind it and the mask crops it, which is the layer order asked for —
   edge in front, drawing beneath, wall behind — and it means no rectangular boundary can
   ever be visible, because the rectangle now ends outside the shape that is shown.

   Cached per size, and derived only from the size, so the clean bedroom and the polluted
   one cannot get different masks and a crossfade between them cannot mismatch.
   ------------------------------------------------------------------------- */
const TORN = { key:"", cv:null, bkey:"", buf:null };
/* its own scratch canvas rather than the washing line's two, which are cached by size for a
   different scene and would only thrash against this one */
function tornBuf(w, h){
  const key = w+"x"+h;
  if (TORN.bkey === key && TORN.buf) return TORN.buf;
  const c = document.createElement("canvas");
  c.width = w; c.height = h;
  TORN.bkey = key; TORN.buf = c;
  return c;
}
function tornPaper(w, h){
  w = Math.max(8, Math.round(w)); h = Math.max(8, Math.round(h));
  const key = w+"x"+h;
  if (TORN.key === key && TORN.cv) return TORN.cv;
  const c = document.createElement("canvas");
  c.width = w; c.height = h;
  const g = c.getContext("2d");
  /* how far the edge wanders, as a fraction of the short side. Small: this is a torn
     sheet, not a burnt one. */
  const A = Math.min(w, h) * 0.030;
  const pt = (u, side) => {
    /* `side` 0 top, 1 right, 2 bottom, 3 left. The sides a child would have cut are
       steadier than the bottom, which is the corner that lifts off a wall. */
    const amp = A * (side === 2 ? 1.35 : side === 0 ? 0.85 : 0.55);
    const n = (hash(u*7.3 + side*31.7)-0.5) * 1.5
            + (hash(u*19.1 + side*11.3)-0.5) * 0.8;
    const d = n * amp;
    if (side === 0) return [u*w, Math.max(0, A*1.2 + d)];
    if (side === 1) return [w - Math.max(0, A*1.2 + d), u*h];
    if (side === 2) return [(1-u)*w, h - Math.max(0, A*1.2 + d)];
    return [Math.max(0, A*1.2 + d), (1-u)*h];
  };
  g.beginPath();
  const STEP = 22;
  for (let side=0; side<4; side++){
    for (let i=0; i<=STEP; i++){
      const p = pt(i/STEP + side, side);
      if (side===0 && i===0) g.moveTo(p[0], p[1]); else g.lineTo(p[0], p[1]);
    }
  }
  g.closePath();
  g.fillStyle = "#fff"; g.fill();
  TORN.key = key; TORN.cv = c;
  return c;
}
...
function drawTapedDrawing(t, rev, air){
  ...
  /* the room's own light, on it — the same warm lamp everything else is under,
     ...
     It goes down OVERSIZE and is then cropped by the torn edge. The overscale is what
     makes the crop safe: the drawing's own rectangle now ends well outside the shape the
     wall shows, so no straight boundary of it can appear at any size, and the illustrated
     part still fills the opening rather than leaving a gap inside it. */
  const OVER = 1.075;
  const pw = w*OVER, ph = h*OVER;
  const mask = tornPaper(w, h);
  const buf = tornBuf(Math.max(8,Math.round(w)), Math.max(8,Math.round(h)));
  const bg = buf.getContext("2d");
  bg.setTransform(1,0,0,1,0,0);
  bg.clearRect(0,0,buf.width,buf.height);
  bg.drawImage(litSprite(PAPER, "paper", pw, ph, 1-lit, 0.52),
               (w-pw)/2, (h-ph)/2, pw, ph);
  bg.globalCompositeOperation = "destination-in";
  bg.drawImage(mask, 0, 0);
  /* and the wall sits very slightly over the paper at the edge, so the sheet reads as
     being under the surface rather than stuck on top of it. A hair of darkness inside the
     torn line and nothing else: no drop shadow, no blur, no glow. */
  bg.globalCompositeOperation = "source-atop";
  const eg = bg.createLinearGradient(0, 0, 0, h);
  eg.addColorStop(0.00, "rgba(24,16,10,0.20)");
  eg.addColorStop(0.055, "rgba(24,16,10,0)");
  eg.addColorStop(0.945, "rgba(24,16,10,0)");
  eg.addColorStop(1.00, "rgba(24,16,10,0.26)");
  bg.fillStyle = eg; bg.fillRect(0,0,w,h);
  const eh = bg.createLinearGradient(0, 0, w, 0);
  eh.addColorStop(0.00, "rgba(24,16,10,0.16)");
  eh.addColorStop(0.045, "rgba(24,16,10,0)");
  eh.addColorStop(0.955, "rgba(24,16,10,0)");
  eh.addColorStop(1.00, "rgba(24,16,10,0.16)");
  bg.fillStyle = eh; bg.fillRect(0,0,w,h);
  bg.globalCompositeOperation = "source-over";
  ctx.drawImage(buf, 0, 0, w, h, -w/2, -h/2, w, h);
  ctx.restore();
}

/* ============================================================================
   THE TOYS
   Four sprites on the floor of a room that is painted without them.

   Each PNG carries a lot of empty space — between twelve and twenty-four per
   cent of its height is transparent below the wheels. Line the image box up
   with the floor and every toy hovers above it by that much, and by a different
   amount each, which is exactly what they were doing. So the content bounds are
   measured off each sprite's own alpha and it is the CONTENT that is placed:
   the wheels land on the boards, and all four end up the same size relative to
   each other rather than the same size relative to their padding.

   No drawn shadow. The paintings light this room from one low window and a
   little ellipse under each toy was reading as a sticker's drop shadow, which
   is the opposite of the point. What sits them in the room instead is the light
   itself — the same warm, dim key everything else in here is under.
   ========================================================================== */
const TOYS = [];
/* src, then the content box inside it: x, y, w, h as fractions of the file.
   Measured from the alpha, not guessed. */
const TOY_SRC = [
  { src:"childrenstoyforbedroom.png",  box:[0.2726, 0.1178, 0.5243, 0.7506] },
  { src:"childrenstoyforbedroom2.png", box:[0.1215, 0.1478, 0.6892, 0.6836] },
  { src:"childrenstoyforbedroom3.png", box:[0.1215, 0.2379, 0.7587, 0.5958] },
  { src:"childrensbedroomtoy4.png",    box:[0.1719, 0.1640, 0.6250, 0.6905] }
];
/* the floor, as seen: depth 0 at the far wall, 1 at the bottom of the frame */
function floorDepth(y){ return cl01((y/H - 0.72) / 0.30); }
function floorScale(y){ return 0.74 + 0.46*floorDepth(y); }

function buildToys(){
  TOYS.length = 0;
  // all four on bare boards: clear of the bed, clear of the rug, clear of the pot
  const rest = [ {x:0.302,y:0.898}, {x:0.352,y:0.962}, {x:0.812,y:0.884}, {x:0.892,y:0.936} ];
  for (let i=0;i<TOY_SRC.length;i++){
    TOYS.push({ src:TOY_SRC[i].src, box:TOY_SRC[i].box, x:rest[i].x, y:rest[i].y,
                w:0.052 + hash(i*6.1)*0.009, rot:(hash(i*2.7)-0.5)*0.10,
...
function toyRect(tt){
  ...
  const bw = tt.box[2]*im.naturalWidth, bh = tt.box[3]*im.naturalHeight;
  const w = tt.w*W*sc, h = w * (bh/bw);
  return { im, x: tt.x*W + c.x, y, w, h, sc,
           sx: tt.box[0]*im.naturalWidth, sy: tt.box[1]*im.naturalHeight, sw: bw, sh: bh };
}
function drawOneToy(tt, t, lit){
  ...
  ctx.rotate(tt.rot + tt.held*Math.sin(t*3.1)*0.02);
  /* Warm and dim, because that is what the room is — and dimmer the further it
     stands from the window, because one low sun through one window is the only
     light in here and the corners of the floor are not in it. */
  const away = cl01(Math.abs(tt.x - 0.52)/0.38);
  const dark = cl01((1-lit)*0.85 + 0.16 + away*0.26);
  ctx.drawImage(litSpriteBox(r.im, tt.src, r, dark, 0.62), -r.w*0.5, -r.h, r.w, r.h);
  ctx.restore();
}
/* the same lighting buffer, but taking a sub-rect of the source */
function litSpriteBox(img, key, r, dark, warm){
  const q = Math.round(dark*24)+"|"+(r.w|0)+"x"+(r.h|0)+"|"+Math.round(warm*12);
  let e = TINTED[key];
  if (e && e.q === q) return e.cv;
  const cv2 = (e && e.cv) || document.createElement("canvas");
  const cw = Math.max(1, Math.ceil(r.w)), ch = Math.max(1, Math.ceil(r.h));
  if (cv2.width !== cw || cv2.height !== ch){ cv2.width = cw; cv2.height = ch; }
  const g = cv2.getContext("2d");
  g.clearRect(0,0,cw,ch);
  g.globalCompositeOperation = "source-over";
  g.drawImage(img, r.sx, r.sy, r.sw, r.sh, 0, 0, cw, ch);
  if (warm > 0){
    g.globalCompositeOperation = "multiply";
    g.fillStyle = rgba([255,188,116], warm);
    g.fillRect(0,0,cw,ch);
    // and put the sprite's own alpha back, or the margin stays as a warm box
    g.globalCompositeOperation = "destination-in";
    g.drawImage(img, r.sx, r.sy, r.sw, r.sh, 0, 0, cw, ch);
  }
  if (dark > 0.002){
    g.globalCompositeOperation = "source-atop";
    g.fillStyle = rgba([14,7,1], dark);
    g.fillRect(0,0,cw,ch);
  }
  g.globalCompositeOperation = "source-over";
  TINTED[key] = { q, cv: cv2 };
...
function toysInteract(t, dt){
  ...
  if (P.down && P.active){
    if (!held){
      ...
      for (let i=TOYS.length-1;i>=0;i--){
        ...
        if (!r) continue;
        if (Math.abs(P.x-r.x) < r.w*0.62 && P.y < r.y+r.h*0.16 && P.y > r.y-r.h*1.06){
  ...
  if (!P.down && !held){
    for (const tt of TOYS){
      const r = toyRect(tt);
      if (r && Math.abs(P.x-r.x) < r.w*0.62 && P.y < r.y && P.y > r.y-r.h){
