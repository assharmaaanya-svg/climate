...
/* ---------------------------------------------------------------------------
   THE EDGE OF THE PAPER, AND WHERE THE OPENING ACTUALLY IS.

   There IS an opening, and it took looking at the thing at four times size to find it: it
   is PAINTED INTO THE WALL, not drawn in code. The artist's bedroom already has a rough
   pale patch on that wall with torn-looking edges and painted tape at its corners — the
   place a drawing is meant to go. Nothing in the code knew about it.

   So the sheet was being drawn INSIDE it and too small: inset by 5.5% of the width and
   7.5% of the height, which left the painted backing showing all the way round, worst on
   the right, and the artwork's own boundary sitting outside the sheet's boundary. Two
   edges, one inside the other, which is exactly the "doubled imagery" and "underlying
   artwork peeking around the wrong places" this kept being described as.

   The fix is to stop inventing a boundary and use the one the painting already has. The
   sheet is drawn OVER the painted patch, a little larger than it rather than smaller, so
   the backing cannot show anywhere; the painted torn edge and the painted tape stay
   visible around it because they are outside the sheet, in the wall, where they always
   were. There is no procedural mask any more — an added torn edge was a second irregular
   boundary competing with the real one, and cutting across it.
   ------------------------------------------------------------------------- */
function drawTapedDrawing(t, rev, air){
  if (!paperBuilt) return;
  if (window.__noPaper) return;      // measurement hook: the wall without the sheet on it
  const c = roomCam(CAM_WALL);
  /* NEGATIVE inset: the sheet is drawn a little LARGER than the painted patch rather than
     smaller, so its own rectangle ends underneath the painted torn edge and the pale
     backing behind it cannot show anywhere. Measured off a 4x enlargement of the rendered
     wall, the sheet was falling short by about 5 px on the left, 7 on the right and 3 at
     the top of a 196x152 patch. Going from +5.5% to -1% adds about 9 px on each side,
     which covers the shortfall and no more: pushed further (-3.5% was tried) the sheet
     swallows the painted torn edge and the tape as well, and the wall loses the very
     boundary that stops this reading as a rectangle. It is a fraction of the patch rather
     than a pixel count, so it holds at any viewport size. */
  const ix = -(TAPED.x1-TAPED.x0)*0.010, iy = -(TAPED.y1-TAPED.y0)*0.010;
  ...
  /* the room's own light, on it — the same warm lamp everything else is under, otherwise
